...
bpy.ops.wm.stl_import(filepath=str(R/'Campus_Center_Ground_Furnished_1_87p5.stl'))
o=bpy.context.object;o.name='Furnished ground floor - unified support-free solid';o.scale=(.001,.001,.001);bpy.ops.object.transform_apply(location=False,rotation=False,scale=True)
...
s.render.engine='CYCLES';s.cycles.device='CPU';s.cycles.samples=40;s.cycles.use_denoising=True;s.cycles.adaptive_threshold=.035
...
c.location=(.3845,.289,1);c.rotation_euler=(0,0,0);d.ortho_scale=.790426;s.render.resolution_x=2048;s.render.resolution_y=1572
...
s['projection_resolution']=[2048,1572];s['physical_canvas_mm']=[790.426,606.713]
bpy.ops.wm.save_as_mainfile(filepath=str(R/'Campus_Center_Projection_Receiver.blend'))
(R/'receiver_contract.json').write_text(json.dumps({'resolution':[2048,1572],'native_resolution':[1024,786],'center_mm':[384.5,289],'canvas_mm':[790.426,790.426*1572/2048],'projection':'Direct vertical orthographic','floor_faces':len(floor_faces),'furniture_faces':len(furniture_faces),'wall_tops_black':True,'upper_floor_or_roof':False},indent=2))
