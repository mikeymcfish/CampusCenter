...
dest='/Game/Campus/ArchitectFinishesV03'
if not ed.does_directory_exist(dest+'/Architect_Finishes_v03'):
 task=unreal.AssetImportTask();task.filename=str(root.parent/'architect_finishes_v01/Architect_Finishes_v03.glb');task.destination_path=dest;task.automated=True;task.save=True
...
for index,slot in enumerate(mesh.static_materials):
 ...
 me.connect_material_property(custom(code),'',unreal.MaterialProperty.MP_BASE_COLOR)
...
for a in actors.get_all_level_actors():
 ...
 if isinstance(a,unreal.RectLight):
  a.light_component.set_editor_property('temperature',5200.0)
 if isinstance(a,unreal.PostProcessVolume):
  s=a.settings
  for k,v in {'auto_exposure_bias':1.0,'white_temp':6500.0,'vignette_intensity':0.0}.items():
...
(root.parent/'architect_finishes_v01/round2_changes.json').write_text(json.dumps(report,indent=2))
unreal.log('ARCHITECT_ROUND2_COMPLETE')
