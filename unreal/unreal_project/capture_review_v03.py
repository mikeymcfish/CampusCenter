...
root=pathlib.Path(unreal.Paths.project_dir());unreal.SystemLibrary.execute_console_command(None,'r.HighResScreenshotDelay 64');out=root/'ReviewV03';out.mkdir(exist_ok=True)
...
cameras[1].set_actor_location(unreal.Vector(900,-1450,165),False,False)
...
def tick(dt):
 ...
 state['task']=unreal.AutomationLibrary.take_high_res_screenshot(1920,1080,str(filename),camera=camera,delay=5.0)
 state['busy']=False
